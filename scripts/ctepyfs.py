...
import ctepy.ctepyfunctions as ctef

# dictionary keys that are used to read from the input files and construct the output files
switcheskeys = ['RFswitch', 'betatronSwitch', 'raddampSwitch', 'IBSswitch', 'collimationSwitch', 'blowupSwitch',
                'collisionSwitch', 'writeAllCoordSwitch', 'writeMountSwitch']
ringparamskeys = ['aatomb2', 'qatomb2', 'aatomb2', 'qatomb2', 'proton', 'ion', 'thib', 'nturns', 'nMacro', 'timeRatio',
                  'nwrite', 'gammat', 'circ', 'gamma1',
                  'gamma2', 'vrfb2', 'nharmb2', 'vrf2b2', 'nharm2b2', 'vrfb2', 'nharmb2', 'vrf2b2', 'nharm2b2',
                  'tunex1', 'tuney1', 'chromx1', 'chromy1',
                  'tunex2', 'tuney2', 'chromx2', 'chromy2', 'dqmin1', 'dqmin2', 'k2L1', 'k2Lskew2', 'k2L2', 'k2Lskew1',
                  'tfsb2', 'tfsb2']
...
colldictkeys = ['refEmxy', 'cutoffAmpl', 'collimAvgSwitch', 'emitMethod', 'nSigCutBeta', 'nSigCutMom', 'betaxMom',
                'dispxMom']
...
dfintcols = ['sim.turn', 't(hours)', 'N1_macro', 'N1_real', 'NlostLum1', 'SumL1', 'NlostDebunch1', 'SumD1', 'NLostBet1',
             'Sumb2', 'NlostMom1', 'SumM1']
...
hglassfacOld, betaSMin, betar1, betar2, vrev1, vrev2, trev1, trev2, trf1, trf2, frf1, frf2, omegarf1, omegarf2, eta1, eta2, eqtime1, eqtime2 = setparam.set_parameters1(
    ringparams, collisiondict)
...
df1 = df1.assign(rmsDelta=lambda row: ctef.getsigpfromvrfandsigs(row['blen'], trev1, ringparams['nharm2b1'],
                                                                 -1 * ringparams['vrf2b1'], eta1, betar1, energy))
df2 = df2.assign(rmsDelta=lambda row: ctef.getsigpfromvrfandsigs(row['blen'], trev2, ringparams['nharm2b2'],
                                                                 -1 * ringparams['vrf2b2'], eta2, betar2, energy))
...
betax1, betay1, betax2, betay2, pcoeff1, tcoeff1, rcoeff1, pcoeff2, tcoeff2, rcoeff2, dgamma_hat1, dgamma_hat2, tunes1, tunes2, tunesexact1, tunesexact2, dftfs1, dftfs2 = setparam.set_parameters2(
    ringparams, startingconditions, omegarf1, trev1, eta1, betar1, omegarf2, trev2, eta2, betar2)
...
if (switches['raddampSwitch'] == True) & (radiationdamping['radMethod'] != 'manual'):
    ...
    if (radiationdamping['radMethod'] == "approx"):
        ...
        print 'I21={:.6e}, I31={:.6e}, I4x1={:.6e}, I4y1={:.6e}, I5x1={:.6e}, I5y1={:.6e}'.format(I21, I31, I4x1, I4y1,
                                                                                                  I5x1, I5y1)
        print 'I22={:.6e}, I32={:.6e}, I4x2={:.6e}, I4y2={:.6e}, I5x2={:.6e}, I5y2={:.6e}'.format(I22, I32, I4x2, I4y2,
                                                                                                  I5x2, I5y2)
    ...
    tradperp1, tradlong1, Cq1, sigE0E01, siglong1, Jx1, Jy1, sigperp1 = setrad.raddampingconstfromI(I21, I31, I4x1,
                                                                                                    I4y1, I5x1, I5y1, 1,
                                                                                                    CalphaE3C1,
                                                                                                    ringparams)
    tradperp2, tradlong2, Cq2, sigE0E02, siglong2, Jx2, Jy2, sigperp2 = setrad.raddampingconstfromI(I22, I32, I4x2,
                                                                                                    I4y2, I5x2, I5y2, 2,
                                                                                                    CalphaE3C2,
                                                                                                    ringparams)
...
for b in b1:
    t, pt, x, px, y, py = ctef.addparticles(df1[df1['bucket'] == b], startingconditions['tauhat1'], tcoeff1, trev1,
                                            v001,
                                            fnharmb1, fnharm2b1, ringparams['vrfb1'], ringparams['vrf2b1'],
                                            ringparams['gamma1'],
                                            startingconditions['bunchLenPrecis'], ringparams['nMacro'], 3,
                                            colldict['cutoffAmpl'], betax1, betay1)
...
for b in b2:
    t, pt, x, px, y, py = ctef.addparticles(df2[df2['bucket'] == b], startingconditions['tauhat2'], tcoeff2, trev2,
                                            v002,
                                            fnharmb2, fnharm2b2, ringparams['vrfb2'], ringparams['vrf2b2'],
                                            ringparams['gamma2'],
                                            startingconditions['bunchLenPrecis'], ringparams['nMacro'], 3,
                                            colldict['cutoffAmpl'], betax2, betay2)
...
for b in b1:
    b1exdict[b], b1eydict[b] = fgetemit.getemittance(b1dfparticles[b]['x'].values, b1dfparticles[b]['y'].values,
                                                     ...
                                                     betax1, betay1, xcut1, ycut1, colldict['cutoffAmpl'],
                                                     colldict['emitMethod'], colldict['refEmxy'])
...
for b in b2:
    b2exdict[b], b2eydict[b] = fgetemit.getemittance(b2dfparticles[b]['x'].values, b2dfparticles[b]['y'].values,
                                                     ...
                                                     betax2, betay2, xcut2, ycut2, colldict['cutoffAmpl'],
                                                     colldict['emitMethod'], colldict['refEmxy'])
...
if (switches['writeMountSwitch']):
    ctef.writedistr(b1avgline, b1avglinex, b1avgliney, b1exdict, b1eydict, ringparams['nMacro'],
                    startingconditions['npart1'], ringparams['qatomb1'],
                    ringparams['nwrite'], kturns, ringparams['thib'], ibsdict['nbins'], betax1, betay1)
    ctef.writedistr(b2avgline, b2avglinex, b2avgliney, b2exdict, b2eydict, ringparams['nMacro'],
                    startingconditions['npart2'], ringparams['qatomb2'],
                    ringparams['nwrite'], kturns, ringparams['thib'], ibsdict['nbins'], betax2, betay2)

# write emittances
for b in b1:
    b1emitdict[b] = cwrite.writeEmi(b1exdict[b], b1eydict[b], b1dfparticles[b]['t'].values,
                                    b1dfparticles[b]['pt'].values, betar1, ringparams['gamma1'],
                                    ringparams['aatomb1'], ringparams['qatomb1'], eqtime, kturns, ringparams['nturns'],
                                    b1emitdict[b])
for b in b2:
    b2emitdict[b] = cwrite.writeEmi(b2exdict[b], b2eydict[b], b2dfparticles[b]['t'].values,
                                    b2dfparticles[b]['pt'].values, betar2, ringparams['gamma2'],
                                    ringparams['aatomb2'], ringparams['qatomb2'], eqtime, kturns, ringparams['nturns'],
                                    b2emitdict[b])
...
for kturns in range(1, int(ringparams['nturns']) + 1):
    ...
    if (switches['RFswitch']):
        for b in b1:
            b1np[b], x, px, y, py, t, pt, b1nLostDebunch[b], kept = frfupdate.rfupdate(b1dfparticles[b]['x'].values,
                                                                                       b1dfparticles[b]['px'].values,
                                                                                       b1dfparticles[b]['y'].values,
                                                                                       b1dfparticles[b]['py'].values,
                                                                                       b1dfparticles[b]['t'].values,
                                                                                       b1dfparticles[b]['pt'].values,
                                                                                       fmix1, b1nLostDebunch[b],
                                                                                       2 * np.pi / trev1, v001,
                                                                                       ringparams['thib'],
                                                                                       tcoeff1, fnharmb1, fnharm2b1,
                                                                                       ringparams['vrfb1'],
                                                                                       ringparams['vrf2b1'])
        ...
        for b in b2:
            b2np[b], x, px, y, py, t, pt, b2nLostDebunch[b], kept = frfupdate.rfupdate(b2dfparticles[b]['x'].values,
                                                                                       b2dfparticles[b]['px'].values,
                                                                                       b2dfparticles[b]['y'].values,
                                                                                       b2dfparticles[b]['py'].values,
                                                                                       b2dfparticles[b]['t'].values,
                                                                                       b2dfparticles[b]['pt'].values,
                                                                                       fmix1, b2nLostDebunch[b],
                                                                                       2 * np.pi / trev2, v002,
                                                                                       ringparams['thib'],
                                                                                       tcoeff2, fnharmb2, fnharm2b2,
                                                                                       ringparams['vrfb2'],
                                                                                       ringparams['vrf2b2'])
    ...
    if (switches['betatronSwitch']):
        for b in b1:
            fsptranschrom.sptranschrom(b1dfparticles[b]['x'].values, b1dfparticles[b]['px'].values,
                                       b1dfparticles[b]['y'].values,
                                       b1dfparticles[b]['py'].values, b1dfparticles[b]['pt'].values,
                                       psix1, psiy1, coeffchromx1, coeffchromy1, tpdqmin1, ringparams['k2L1'],
                                       ringparams['k2Lskew1'])

            b1avglinex[b], b1avgliney[b] = fkeeptransprof.keeptransvprof(b1avglinex[b], b1avgliney[b], x, y,
                                                                         b1exdict[b], b1eydict[b], betax1, betay1)

        for b in b2:
            fsptranschrom.sptranschrom(b2dfparticles[b]['x'].values, b2dfparticles[b]['px'].values,
                                       b2dfparticles[b]['y'].values,
                                       b2dfparticles[b]['py'].values, b2dfparticles[b]['pt'].values,
                                       psix2, psiy2, coeffchromx2, coeffchromy2, tpdqmin2, ringparams['k2L2'],
                                       ringparams['k2Lskew2'])

            b2avglinex[b], b2avgliney[b] = fkeeptransprof.keeptransvprof(b2avglinex[b], b2avgliney[b], x, y,
                                                                         b2exdict[b], b2eydict[b], betax2, betay2)

    #  radiation damping and quantum excitation
    if (switches['raddampSwitch']):
        for b in b1:
            x, px, y, py, pt, iseed, b1np[b] = frad.raddamp(b1dfparticles[b]['x'].values, b1dfparticles[b]['px'].values,
                                                            b1dfparticles[b]['y'].values,
                                                            b1dfparticles[b]['py'].values,
                                                            b1dfparticles[b]['pt'].values,
                                                            tradlong1, tradperp1, trev1, siglong1, sigperp1,
                                                            ringparams['timeRatio'], 12763)
        for b in b2:
            x, px, y, py, pt, iseed, b2np[b] = frad.raddamp(b2dfparticles[b]['x'].values, b2dfparticles[b]['px'].values,
                                                            b2dfparticles[b]['y'].values,
                                                            b2dfparticles[b]['py'].values,
                                                            b2dfparticles[b]['pt'].values,
                                                            tradlong2, tradperp2, trev2, siglong2, sigperp2,
                                                            ringparams['timeRatio'], 12763)

    # ibs
    if (switches['IBSswitch']):
        for b in b1:
            px, py, t, pt, ibsrow, iseed = fibs.ibslong(b1pnumber[b], b1dfparticles[b]['px'].values,
                                                        b1dfparticles[b]['py'].values, b1dfparticles[b]['t'].values,
                                                        b1dfparticles[b]['pt'].values, b1exdict[b], b1eydict[b],
                                                        ringparams['nMacro'], ringparams['circ'],
                                                        iwrite, ringparams['thib'], b1avgline[b], betar1,
                                                        ringparams['gamma1'],
                                                        ringparams['gammat'], betax1, betay1, ringparams['aatomb1'],
                                                        ringparams['qatomb1'],
                                                        trev1, np.array(dftfs1['BETX'].values, order='F'),
                                                        np.array(dftfs1['BETY'].values, order='F'),
                                                        np.array(dftfs1['DX'].values, order='F'),
                                                        np.array(dftfs1['DY'].values, order='F'),
                                                        np.array(dftfs1['DPX'].values, order='F'),
                                                        np.array(dftfs1['DPY'].values, order='F'),
                                                        np.array(dftfs1['ALFX'].values, order='F'),
                                                        np.array(dftfs1['ALFY'].values, order='F'),
                                                        np.array(dftfs1['L'].values, order='F'), gtab,
                                                        ibsdict['coulombLog'], xmin, xmax, ymin, ymax, zmin, zmax, ap,
                                                        ax, ay,
                                                        pnuminterp, ibsdict['ibsMethod'], ibsdict['coupleIBS'], nturns,
                                                        kturns, eqtime,
                                                        ibsdict['fracibstot'], const.c, np.pi, ringparams['timeRatio'],
                                                        12673)
            if (iwrite == 1):
                b1ibsdict[b] = b1ibsdict[b].append(pd.DataFrame(ibsrow.reshape((1, 5)), columns=b1ibsdict[b].columns))
        for b in b2:
            px, py, t, pt, ibsrow, iseed = fibs.ibslong(b2pnumber[b], b2dfparticles[b]['px'].values,
                                                        b2dfparticles[b]['py'].values, b2dfparticles[b]['t'].values,
                                                        b2dfparticles[b]['pt'].values, b2exdict[b], b2eydict[b],
                                                        ringparams['nMacro'], ringparams['circ'],
                                                        iwrite, ringparams['thib'], b2avgline[b], betar2,
                                                        ringparams['gamma2'],
                                                        ringparams['gammat'], betax2, betay2, ringparams['aatomb2'],
                                                        ringparams['qatomb2'],
                                                        trev2, np.array(dftfs2['BETX'].values, order='F'),
                                                        np.array(dftfs2['BETY'].values, order='F'),
                                                        np.array(dftfs2['DX'].values, order='F'),
                                                        np.array(dftfs2['DY'].values, order='F'),
                                                        np.array(dftfs2['DPX'].values, order='F'),
                                                        np.array(dftfs2['DPY'].values, order='F'),
                                                        np.array(dftfs2['ALFX'].values, order='F'),
                                                        np.array(dftfs2['ALFY'].values, order='F'),
                                                        np.array(dftfs2['L'].values, order='F'), gtab,
                                                        ibsdict['coulombLog'], xmin, xmax, ymin, ymax, zmin, zmax, ap,
                                                        ax, ay,
                                                        pnuminterp, ibsdict['ibsMethod'], ibsdict['coupleIBS'], nturns,
                                                        kturns, eqtime,
    ...
    if (switches['collimationSwitch']):
        for b in b1:
            collx, colly, momx, kept = fcollim.collimation(b1dfparticles[b]['x'].values, b1dfparticles[b]['px'].values,
                                                           b1dfparticles[b]['y'].values,
                                                           b1dfparticles[b]['py'].values, b1dfparticles[b]['t'].values,
                                                           b1dfparticles[b]['pt'].values,
                                                           b1nLostMom[b], b1nLostBeta[b], eqtime, betax1, betay1,
                                                           colldict['refEmxy'], colldict['collimAvgSwitch'],
                                                           colldict['nSigCutBeta'], colldict['nSigCutMom'],
                                                           colldict['betaxMom'], betar1, ringparams['gamma1'],
        ...
        for b in b2:
            collx, colly, momx, kept = fcollim.collimation(b2dfparticles[b]['x'].values, b2dfparticles[b]['px'].values,
                                                           b2dfparticles[b]['y'].values,
                                                           b2dfparticles[b]['py'].values, b2dfparticles[b]['t'].values,
                                                           b2dfparticles[b]['pt'].values,
                                                           b2nLostMom[b], b2nLostBeta[b], eqtime, betax2, betay2,
                                                           colldict['refEmxy'], colldict['collimAvgSwitch'],
                                                           colldict['nSigCutBeta'], colldict['nSigCutMom'],
                                                           colldict['betaxMom'], betar2, ringparams['gamma2'],
                                                           colldict['dispxMom'], kturns, nturns)
    ...
    if (switches['blowupSwitch']):
        for b in b1:
            px, py, iseed = fblowup.blowup(b1dfparticles[b]['px'].values, b1dfparticles[b]['py'].values, 12673,
                                           blowupdict['blowupMethod'], betax1, betay1, colldict['refEmxy'],
        ...
        for b in b2:
            px, py, iseed = fblowup.blowup(b2dfparticles[b]['px'].values, b2dfparticles[b]['py'].values, 12673,
                                           blowupdict['blowupMethod'], betax2, betay2, colldict['refEmxy'],
                                           blowupdict['pxKickFac'], blowupdict['pyKickFac'])
    # collision
                    ...
                    if tdc[k][i] is not None:
                        ...
                        if i == 0:
                            ...
                            hglassfac, betass1['b1-' + str(b1bunch) + '-b2-' + str(b2bunch)], kept1, kept2, lum1out = \
                                fcollision.collision6a(collisiondict['sigI'], ringparams['thib'],
                                                       collisiondict['longIntBins'],
                                                       ringparams['timeRatio'], ringparams['circ'], np.pi, const.c,
                                                       12673, hgold,
                                                       ...
                                                       b1exdict[b1bunch], b1eydict[b1bunch], betax1, betay1, bs1,
                                                       lumimax1, theta1,
                        ...
                        if i == 1:
                            ...
                            hglassfac, betass2['b1-' + str(b1bunch) + '-b2-' + str(b2bunch)], kept1, kept2, lum2out = \
                                fcollision.collision6a(collisiondict['sigI'], ringparams['thib'],
                                                       ...
                                                       b1dfparticles[b1bunch]['t'].values,
                                                       b1dfparticles[b1bunch]['pt'].values,
                                                       b1exdict[b1bunch], b1eydict[b1bunch], betax1, betay1, bs2,
                                                       lumimax2, theta2,
                                                       b2pnumber[b2bunch].values[0],
                                                       b2dfparticles[b2bunch]['x'].values,
                                                       b2dfparticles[b2bunch]['px'].values,
                        ...
                        if i == 2:
                            ...
                            hglassfac, betass5['b1-' + str(b1bunch) + '-b2-' + str(b2bunch)], kept1, kept2, lum5out = \
                                fcollision.collision6a(collisiondict['sigI'], ringparams['thib'],
                                                       ...
                                                       b1dfparticles[b1bunch]['t'].values,
                                                       b1dfparticles[b1bunch]['pt'].values,
                                                       b1exdict[b1bunch], b1eydict[b1bunch], betax1, betay1, bs5,
                                                       lumimax5, theta5,
                                                       b2pnumber[b2bunch].values[0],
                                                       b2dfparticles[b2bunch]['x'].values,
                                                       b2dfparticles[b2bunch]['px'].values,
                        ...
                        if i == 3:
                            ...
                            hglassfac, betass8['b1-' + str(b1bunch) + '-b2-' + str(b2bunch)], kept1, kept2, lum8out = \
                                fcollision.collision6a(collisiondict['sigI'], ringparams['thib'],
                                                       ...
                                                       b1dfparticles[b1bunch]['t'].values,
                                                       b1dfparticles[b1bunch]['pt'].values,
                                                       b1exdict[b1bunch], b1eydict[b1bunch], betax1, betay1, bs8,
                                                       lumimax8, theta8,
                                                       b2pnumber[b2bunch].values[0],
                                                       b2dfparticles[b2bunch]['x'].values,
                                                       b2dfparticles[b2bunch]['px'].values,
    ...
    if ((kturns == 1) & (switches['collisionSwitch'] == 1.0)):
        ...
        for k in lumip1.keys():
            lumip1[k] = cwrite.writeLumi(lum1[k], hglassfacOld['ip1'][ip1counter], lumip1[k], kturns, nturns, eqtime,
                                         betass1[k])
            ip1counter += ip1counter
        ...
        for k in lumip2.keys():
            lumip2[k] = cwrite.writeLumi(lum2[k], hglassfacOld['ip2'][ip2counter], lumip2[k], kturns, nturns, eqtime,
                                         betass2[k])
            ip2counter += ip2counter
        # ip5
        ip5counter = 0
        for k in lumip5.keys():
            lumip5[k] = cwrite.writeLumi(lum5[k], hglassfacOld['ip5'][ip5counter], lumip5[k], kturns, nturns, eqtime,
                                         betass5[k])
            ip5counter += ip5counter
        # ip8
        ip8counter = 0
        for k in lumip8.keys():
            lumip8[k] = cwrite.writeLumi(lum8[k], hglassfacOld['ip8'][ip8counter], lumip8[k], kturns, nturns, eqtime,
                                         betass8[k])
            ip8counter += ip8counter
    #     iwrite=1
    if (iwrite == 1):
        ...
        if switches['writeMountSwitch']:
            for b in b1:
                b1dfwritemount[b] = cwrite.mountainr(ringparams['nMacro'], b1pnumber[b], b1avgline[b],
                                                     ringparams['qatomb1'],
                                                     ringparams['nwrite'], kturns, ringparams['thib'], ibsdict['nbins'],
                                                     b1dfwritemount[b])

                b1dfwritemounttrans[b] = cwrite.mountTransv(b1avglinex[b], b1avgliney[b], b1exdict[b], b1eydict[b],
                                                            betax1, betay1, ibsdict['nbins'],
                                                            kturns, b1dfwritemounttrans[b])
            for b in b2:
                b2dfwritemount[b] = cwrite.mountainr(ringparams['nMacro'], b2pnumber[b], b2avgline[b],
                                                     ringparams['qatomb2'],
                                                     ringparams['nwrite'], kturns, ringparams['thib'], ibsdict['nbins'],
                                                     b2dfwritemount[b])
                b2dfwritemounttrans[b] = cwrite.mountTransv(b2avglinex[b], b2avgliney[b], b2exdict[b], b2eydict[b],
                                                            betax2, betay2, ibsdict['nbins'],
                                                            kturns, b2dfwritemounttrans[b])
        for b in b1:
            b1emitdict[b] = cwrite.writeEmi(b1exdict[b], b1eydict[b], b1dfparticles[b]['t'], b1dfparticles[b]['pt'],
                                            betar1, ringparams['gamma1'],
                                            ringparams['aatomb1'], ringparams['qatomb1'], eqtime, kturns,
                                            ringparams['nturns'], b1emitdict[b])
            b1intdict[b], b1nLostLumSum[b], b1nLostDebunchSum[b], b1nLostBetaSum[b], b1nLostMomSum[b] = \
                cwrite.writeNb(b1dfparticles[b], ringparams['nMacro'], b1pnumber[b].values[0], b1nLostLum[b],
                               b1nLostLumSum[b],
        ...
        for b in b2:
            b2emitdict[b] = cwrite.writeEmi(b2exdict[b], b2eydict[b], b2dfparticles[b]['t'], b2dfparticles[b]['pt'],
                                            betar2, ringparams['gamma2'],
                                            ringparams['aatomb2'], ringparams['qatomb2'], eqtime, kturns,
                                            ringparams['nturns'], b2emitdict[b])
            b2intdict[b], b2nLostLumSum[b], b2nLostDebunchSum[b], b2nLostBetaSum[b], b2nLostMomSum[b] = \
                cwrite.writeNb(b2dfparticles[b], ringparams['nMacro'], b2pnumber[b].values[0], b2nLostLum[b],
                               b2nLostLumSum[b],
        ...
        if (switches['collisionSwitch']):
            ...
            ip1counter = 0
            for k in lumip1.keys():
                lumip1[k] = cwrite.writeLumi(lum1[k], hglassfacOld['ip1'][ip1counter], lumip1[k], kturns, nturns,
                                             eqtime, betass1[k])
                ip1counter += ip1counter
            ...
            ip2counter = 0
            for k in lumip2.keys():
                lumip2[k] = cwrite.writeLumi(lum2[k], hglassfacOld['ip2'][ip2counter], lumip2[k], kturns, nturns,
                                             eqtime, betass2[k])
                ip2counter += ip2counter
            # ip5
            ip5counter = 0
            for k in lumip5.keys():
                lumip5[k] = cwrite.writeLumi(lum5[k], hglassfacOld['ip5'][ip5counter], lumip5[k], kturns, nturns,
                                             eqtime, betass5[k])
                ip5counter += ip5counter
            # ip8
            ip8counter = 0
            for k in lumip8.keys():
                lumip8[k] = cwrite.writeLumi(lum8[k], hglassfacOld['ip8'][ip8counter], lumip8[k], kturns, nturns,
                                             eqtime, betass8[k])
                ip8counter += ip8counter
